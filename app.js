...
}else{pdfCanvas.style.display="none";}
}
...
const doc=new Document({sections:[{children:[new Paragraph(sheetData)]}]});
...
const {Document,Packer,Paragraph}=window.docx;
const doc=new Document({sections:[{children:[new Paragraph(fullText)]}]});
const blob=await Packer.toBlob(doc);
link.href=URL.createObjectURL(blob);
link.download="output.docx";
link.textContent="⬇ Télécharger Word";
...
const pdf=await pdfjsLib.getDocument({data:pdfData}).promise;
let rows=[];
for(let i=1;i<=pdf.numPages;i++){const page=await pdf.getPage(i);const textContent=await page.getTextContent();const line=textContent.items.map(item=>item.str).join(" ");rows.push([line]);}
